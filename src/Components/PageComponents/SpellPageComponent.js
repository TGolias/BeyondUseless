...
export function SpellPageComponent({spell, data, playerConfigs, copyLinkToSpell}) {
    ...
    let range = calculateRange(playerConfigs, additionalEffects, spell.range, { spell, slotLevel: data?.castAtLevel ?? spell.level });
    ...
    return <>
        <div className="spellPageContainer">
            ...
            <div className="spellPageDescription" style={{display: (buffAmount || buffDescription ? "block" : "none")}}>
                <div><b>Buff:</b> {(buffAmount ? buffAmount : "")}{buffDescription ? " " : ""}{parseStringForBoldMarkup(buffDescription)}</div>
            </div>
