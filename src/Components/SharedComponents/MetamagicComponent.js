    ...
    if (metamagicOptions.length > 0) {
        ...
        if (sorceryPoints && sorceryPoints.maxUses > 0) {
            ...
            if (metamagicPointsThatWillBeRemaining > 0) {
                const range = calculateRange(playerConfigs, menuConfig.additionalEffects ?? [], menuConfig.spell.range, { spell: menuConfig.spell, slotLevel: menuConfig.useSpellSlotLevel });
